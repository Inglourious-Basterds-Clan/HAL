...
if (_HQ getVariable [QEGVAR(core,flee), true]) then {
    ...
    private _midCow = 0;
    if !((count _friends) == 0) then {_midCow = _AllCow/(count _friends)};

    {
        ...
        if !(_panic) then {
            ...
            if ((random 1.1) > _cowF) then {
                ...
                if !(_isCaptive) then {_x setVariable [("Busy" + (str _x)), false]};
            };
        };
    } forEach _friends
};
...
if !(_AAO) then {_AAO = _HQ getVariable [QEGVAR(core,forceAAO), false]};
if !(_EBT) then {_EBT = _HQ getVariable [QEGVAR(core,forceEBDoctrine), false]};
