...
if ((_HQ getVariable [QEGVAR(core,lRelocating),false]) && {!(_AAO)}) then {
    ...
    if !(_onMove) then {
        if (!(isPlayer (_HQ getVariable ["leaderHQ",(leader _HQ)])) && {((_cycleC == 1) or {not ((_HQ getVariable [QEGVAR(core,progress),0]) == 0)})}) then {
            ...
            if !(_enemyN) then {
                ...
                if (((_HQ getVariable [QEGVAR(core,lRelocating),false])) && {((_HQ getVariable [QEGVAR(core,nObj),1]) > 1) && {(_cycleC > 1)}}) then {
                    private _code = {
                        params ["_Lpos", "_HQ", "_knownEG"];
                        ...
                        waitUntil {
                            ...
                            if (isNull _HQ) then {
                                ...
                                if !(_getBack) then {
                                    if ((((vehicle (_HQ getVariable ["leaderHQ", (leader _HQ)])) distance _LPos) < 30) || {(_HQ getVariable [QEGVAR(common,kIA), false])}) then {_onPlace = true};
                                };
                            };
                        ...
                        if !(_onPlace) then {
                            ...
                            private _spd = "NORMAL";
                            if !(((vehicle (_HQ getVariable ["leaderHQ",(leader _HQ)])) distance _LPos) < 50) then {_spd = "FULL"};
                            private _wp = [_HQ,_Lpos,"MOVE","AWARE","GREEN",_spd,["true",""],true,_rds,[0,0,0],"FILE"] call EFUNC(common,WPadd);
                            if (isNull (assignedVehicle (_HQ getVariable ["leaderHQ",(leader _HQ)]))) then {
                                if (_HQ getVariable [QEGVAR(core,getHQInside), false]) then {[_wp] call EFUNC(common,goInside)}
                            };
